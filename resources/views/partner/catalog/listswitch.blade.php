          ...
          <table class="table app-cat-table table-striped">
            ...
            <tbody>
              ...
              <tr class="catalogitemtr" data-id="{{ $catalogitem->id }}">
                ...
                <td>Conves.io</td>
                <td>7 days ago</td>
                <!-- <td>{{ $catalogitem->type == 'private' ? 'Private' : '' }} </td> -->
                <td>
                  <div class="form-check form-switch">
                    <input class="form-check-input catalog-item" data-id="{{ $catalogitem->id }}" data-type="{{ $catalogitem->type }}" type="checkbox" role="switch" id="flexSwitchCheckDefault" {{ $catalogitem->status == 1 ? 'checked' : '' }}>
...
<div class="modal ImportingApplicationBlueprint" tabindex="-1">
  <div class="modal-dialog">
    <div class="modal-content">
      <form id="importcatalagform" method="POST">
      @csrf  
        ...
        <div class="modal-body" style="background-color: #F5F5F5;">
          <div style="display: flex;justify-content: space-between;">
            <div class="mb-3" style="width: 250px;">
              ...
              <input type="email" name="src" class="form-control" id="exampleFormControlInput1" placeholder="e.g. git@repo.convesio.com/application">
            </div>
          ...
          <div class="mb-3">
            ...
            <textarea name="ssh" class="form-control" id="exampleFormControlTextarea1" rows="3" placeholder="Copy and paste your private ssh key over here. This is optional and only needed if you are using a private repository for your catalog blueprints"></textarea>
          </div>
          <div class="yelow-box">
            <p style="font-size: 11px;">Allow pinning of version to avoid auto-upgrade or notice for the same</p>
            <div class="form-check form-switch">
              <input name="autoupgrade" class="form-check-input"  type="checkbox" role="switch" id="popupswitch">
        ...
        <div class="modal-footer">
          ...
          <button type="submit" class="btn btn-primary import-app-bluprint">Start Import</button>
        </div>
      </form>
    </div>
  </div>
...
<script>
  $(document).ready(function() {
    ...
    $('.catalog-item').change(function() {
      ...
      console.log(data);
      $.ajax({
        url: '/api/partner/{{ request()->segment(2) }}/catalog/edit',
        type: 'PUT',
        data: data,
    ...
    $('#importcatalagform').on('submit', function(event) {
      event.preventDefault();
      $.ajax({
        url: '/customers/{{ request()->segment(2) }}/import-catalog',
        type: 'POST',
        data: $(this).serialize(),
        success: function(response) {
